...
* **Step 2** — Temporal PD: each `t ↦ ν_t(B)` is semigroup-positive-definite,
  so `semigroup_pd_laplace` (BCR d=0) gives a Laplace representation.
* **Step 3** — Product measure assembly: combine the families via
  `joint_measure_from_temporal_slices` to produce a single product measure on
  `[0,∞) × ℝ^d`.
